...
public class MPXPlayersSyncVerification extends ParentTest{
    ...
    public void MPXPlayersSyncVerification() throws Exception{
        ...
        if (settings.IsMPXConfigured() == true) {
            ...
            if (configuredAccounts.size() >= 4) {
                Integer playerCountForAccount4 = allPlayerTitlesForAccount4.size();
                contentParent.VerifyMessageStatus(playerCountForAccount4.toString() + " players returned for account \"" + configuredAccounts.get(3) + "\"");
            }
            ...
            if (configuredAccounts.size() >= 4) {
                mpxMedia.VerifyImportAccountPlayerOptions(configuredAccounts.get(3), allActivePlayerTitlesForAccount4);
            }
