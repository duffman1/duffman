...
public class ValidateIngestionTimePub7 extends ParentTest{
    ...
    public void Test() throws Exception {
	    ...
	    String assetCreationFilePath = System.getProperty("user.dir") + "/src/test/java/com/nbcuni/test/publisher/contentbuildscripts/MPXPerformanceUpgrade/DeleteVideos/AssetsDeleted.txt";
    		...
    		try {
                ...
                for (String asset : allAssets) {
            	   ...
            	   if (!entriesProcessed.contains(asset)) {
                	   ...
                	   deletionTimeStamp = data[4];
                	   deletionTime = Long.valueOf(data[5]).longValue();
                	   ...
                	   String searchTimeStamp = null;
                   	   ...
                   	   //now search for the asset to be deleted
                   	   long deletionSearchTime = 0;
                   	   String deletionSearchTimeStamp = null;
                   	   while (searchFor.GetFirstMPXMediaSearchResult().equals(mediaTitle)) {
                		   ...
                		   if (!searchFor.GetFirstMPXMediaSearchResult().equals(mediaTitle)) {
                			   deletionSearchTime = System.nanoTime();
                			   deletionSearchTimeStamp = new SimpleDateFormat("MMddyy-hh:mm:ss a").format(Calendar.getInstance().getTime());
                       ...
                       //log the ingestion time (or failed ingestion)
                   	   ...
                   	   if (ingestionTimeout == true) {
                   		   ...
                   		   long deleteTime = deletionSearchTime - deletionTime; 
                   		   ingestionMessage = type + "," + mediaTitle + "," + "Asset Published at " + publishTimeStamp + "," + "Asset Ingested at " + searchTimeStamp + "," + "ELAPSED INGESTION TIME = " + TimeUnit.SECONDS.convert(ingestionTime, TimeUnit.NANOSECONDS) + " seconds" +
                   		   "," + "Asset Deleted in MPX at " + deletionTimeStamp + "," + "Asset Deleted from Pub7 at " + deletionSearchTimeStamp + "," + "ELAPSED DELETION TIME = " + TimeUnit.SECONDS.convert(deleteTime, TimeUnit.NANOSECONDS) + " seconds";
                   		   
                   	   }
                       
                       String assetIngestionFilePath = System.getProperty("user.dir") + "/src/test/java/com/nbcuni/test/publisher/contentbuildscripts/MPXPerformanceUpgrade/DeleteVideos/AssetsIngestedAndDeleted.txt";
