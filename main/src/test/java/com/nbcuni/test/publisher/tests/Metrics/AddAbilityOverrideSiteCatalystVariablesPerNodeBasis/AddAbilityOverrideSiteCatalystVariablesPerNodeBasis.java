...
,Step 9,Logout from P7 ,Logout successful 
    ...
    public void AddAbilityOverrideSiteCatalystVariablesPerNodeBasis_Test() throws Exception {
        	...
        	siteCatalystVariables.EnterBlankVariableValue("[node:content-type]");
        	...
        	if (!pageSource.contains(newVariableName)) {
        		Assert.fail("Page source does not contain site catalyst variable '" + newVariableName + "'.");
        	}
        	
        	
        	Assert.fail("Test under construction. following up with Kumud for details on step 7");
